...
from src.utils.frequency_utils import get_fft_of_DataArray, scale_fft_amplitude, complex_to_polar
from src.utils.utils import dict_to_df_meshgrid
from src.visualisation.NMF_visualization import plot_outlier_events
...
def NMF_sensitivity_analysis(hyperparameter, out_path):
    ...
    analysis = SensitivityAnalysis(result_path=out_path,
                                   event_identifiers=ds.event.values[~bool_test])
    for index, row in df_meshgrid.iterrows():
        ...
        nmf_result = NMFResult(out_path=out_path,
                               ...
                               **row.to_dict())
        nmf_result.calculate_p_values(da_fft_amp.data[~bool_test], plot_fit=True, beta=row["beta_loss"])
        nmf_result.save()
        analysis.add_result(nmf_result)

        # plot components
        # plot_NMF_components(da_fft_amp.frequency, nmf_model.components_, experiment_path / 'components.png')

        # free allocated storage
        plt.close('all')
        del ds_detrend, da_processed, da_fft, da_fft_amp, da_fft_phase, na_fft_flat
        gc.collect()
...
if __name__ == "__main__":
    ...
    mp3_fpa_df_unique = mp3_fpa_df.drop_duplicates(
        ...
        subset=['fpa_identifier'])  # [mp3_fpa_df['timestamp_fgc'] >= 1526582397220000000]
