...
if __name__ == "__main__":
    ...
    # Define position map
    pos_map = rb_magnet_metadata[["Circuit", "Magnet", "#Electric_circuit", "phys_pos"]].rename(
        columns={"#Electric_circuit": "El. Position",
                 "phys_pos": "Phys. Position"})
    ...
    for q in pos_map[pos_map.Circuit == 'RB.A12']["Phys. Position"].values:
        for circuit in pos_map["Circuit"].unique():
            el_quench_pos = \
            map_position_index(pos_map, old_index=np.array(q, dtype=int), origin="El. Position", to="Phys. Position",
